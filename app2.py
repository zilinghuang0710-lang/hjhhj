...
FALLBACK_STOCKS = [
    ...
    {"code":"600763","name":"通策医疗","sector":"医疗"},
    # 约 500 只，实际部署时可继续扩充
]
...
class StockAnalyzer:
    ...
    def _try_tencent_daily(self):
        ...
        try:
            resp = http_session.get(tx_url, timeout=1.5)
            if resp.status_code == 200:
                data = resp.json()
    ...
    def _try_eastmoney_daily(self, end_date=None):
        ...
        headers = {"Referer": "http://quote.eastmoney.com/"}
        try:
            resp = http_session.get(url_em, params=params_em, headers=headers, timeout=1.5)
            if resp.status_code == 200:
                data = resp.json()
    ...
    def _fetch_weekly_data_fast(self):
        """周线获取：东财1秒超时，失败立即用日线合成"""
        mkt = "0" if self.symbol.startswith(("0","3")) else "1"
        ...
        try:
            resp = http_session.get(url, params=params, timeout=1)
            if resp.status_code == 200:
                data = resp.json()
                if data and data.get("data") and data["data"].get("klines"):
                    klines = data["data"]["klines"]
...
<body>
<nav class="navbar navbar-dark bg-dark mb-3"><div class="container-fluid"><span class="navbar-brand text-success fw-bold">🛰️ PRO-QUANT V11 终极版</span></div></nav>
...
<script>
    ...
    async function analyze() {
        ...
        const timeout = setTimeout(() => controller.abort(), 7000);  // 7秒超时
        try {
