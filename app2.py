...
def get_active_stocks(limit=400):
    ...
    # 兜底：如果前两个接口都挂了，注入3000只全量
    if not stocks:
        stocks = generate_fallback_stocks()
        limit = 3000 # 如果启用兜底，就扩大限制
...
class StockAnalyzer:
    ...
    def _fetch_data_inner(self, end_date=None):
        ...
        try:
            resp = http_session.get(url_em, params=params_em, headers={"Referer": "https://quote.eastmoney.com/"}, timeout=(1.5, 2))
            if resp.status_code == 200:
                data = resp.json()
    ...
    def calculate_indicators(self):
        ...
        # 修复图表丢失的核心：清除可能导致 JSON 序列化崩溃的 NaN / Inf
...
def scan_stocks(strategy, top_n=10):
    stocks = get_active_stocks(3000) # 若启用兜底可能高达3000
    results = []
    ex = ThreadPoolExecutor(max_workers=40) # 加大并发
    ...
    try:
        for f in as_completed(futures.keys()):
            ...
            if res:
                results.append(res)
                # 核心熔断逻辑：只要凑够15只股票立马返回！不等待3000只扫完！极速出结果！
...
<head>
    ...
    <title>PRO-QUANT V11 全新极速扫描修复版</title>
    ...
    <style>
        ...
        .terminal-box { background: #010409; color: #ffffff; font-family: 'Consolas', monospace; padding: 15px; border-radius: 8px; border: 1px solid var(--border); font-size: 14px; white-space: pre-wrap; }
        .terminal-box.green-text { color: #39ff14; }
...
<nav class="navbar navbar-dark bg-dark mb-3 border-bottom border-secondary">
    <div class="container-fluid"><span class="navbar-brand fw-bold text-success">🛰️ PRO-QUANT V11 (熔断秒出版+全A股防崩)</span></div>
</nav>
        ...
        <div class="col-lg-3">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <input type="date" id="bt_date" class="form-control form-control-sm mb-1 force-white">
...
<script>
    ...
    async function analyze() {
        ...
        const timeout = setTimeout(() => controller.abort(), 12000); // 增加诊断容错时间
        try {
            ...
            <div class="mt-4">
                ...
                <div class="collapse show mt-2" id="textReport">
                    <pre class="terminal-box green-text">${r.text_report}</pre>
                </div>
            </div>`;
            document.getElementById('displayArea').innerHTML = html;
            
            setTimeout(() => { renderChart(r.chart_data); }, 150);
    ...
    async function scan(strategy) {
        document.getElementById('loader').style.display = 'block';
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), 45000); // 放宽扫描等待时间到45秒，因为后端有熔断
        try {
            ...
            clearTimeout(timeout);
            document.getElementById('loader').style.display = 'none';
            document.getElementById('displayArea').innerHTML = '<div class="alert alert-warning">⏰ 扫描请求超时，请检查网络或后端服务</div>';
        }
    }
    ...
    function addPort() {
        ...
        div.innerHTML = '<input class="form-control form-control-sm p_code force-white" placeholder="代码"><input class="form-control form-control-sm p_cost force-white" placeholder="成本"><input class="form-control form-control-sm p_weight force-white" placeholder="仓位%">';
    ...
    async function calcPort() {
        ...
        const data = await res.json();
        document.getElementById('loader').style.display = 'none';
        document.getElementById('displayArea').innerHTML = `<h5 class="text-white">🛠️ 持仓组合结算</h5><pre class="terminal-box force-white mt-3">${data.report || data.error}</pre>`;
    }
    ...
    async function runBt() {
        ...
        const data = await res.json();
        document.getElementById('loader').style.display = 'none';
        document.getElementById('displayArea').innerHTML = `<h5 class="text-white">⏳ 历史回测报告</h5><pre class="terminal-box force-white mt-3">${data.report || data.error}</pre>`;
    }
</script>
</body>
</html>'''
...
if __name__ == '__main__':
    print("✅ PRO-QUANT V11 (熔断极速防卡版) 已启动，确保全功能平稳运行！")
